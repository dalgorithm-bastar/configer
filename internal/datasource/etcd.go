...
func (e *EtcdType) ConnectToEtcd() error {
	...
	e.client, err = clientv3.New(clientv3.Config{
		Endpoints:   viper.GetStringSlice(define.EtcdEndpoints),
		DialTimeout: time.Duration(2*viper.GetInt(define.EtcdOperationTimeout)) * time.Second,
		Username:    viper.GetString(define.EtcdUserName),
		Password:    viper.GetString(define.EtcdPassWord),
	})
	if err != nil {
		return err
	}
...
func (e *EtcdType) Put(key, value string) error {
	ctx, cancel := context.WithTimeout(context.TODO(), time.Duration(viper.GetInt(define.EtcdOperationTimeout))*time.Second)
...
func (e *EtcdType) AtomicCommit(putMap map[string]string, deleteSlice []string) error {
	...
	ctx, cancel := context.WithTimeout(context.TODO(), time.Duration(3*viper.GetInt(define.EtcdOperationTimeout))*time.Second) //提交时限设为一般时限*提交次数
