...
func TestDirType_AtomicCommit(t *testing.T) {
	type fields struct {
		path string
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if err := c.AtomicCommit(tt.args.putMap, tt.args.deleteMap); (err != nil) != tt.wantErr {
				t.Errorf("AtomicCommit() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
...
func TestDirType_getData(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			name: "ok",
			fields: fields{
				path: "../../test/unittestfiles/config",
				data: nil,
			},
			wantErr: false,
		},
		{
			name: "err",
			fields: fields{
				path: "../../test/unittesetfiles/configcenter.json",
				data: nil,
			},
			wantErr: true,
		},
...
func TestNewDirType(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "ok",
			args: args{path: "../../test/unittestfiles/config"},
			want: &DirType{
				path: "../../test/unittestfiles/config",
  ...
  "log": {
    ...
    "maxage": 30
		...
		{
			name:    "err",
			args:    args{path: "../../test/unittestfiles/config/configcenter.json"},
			want:    nil,
			wantErr: true,
		},
	}
