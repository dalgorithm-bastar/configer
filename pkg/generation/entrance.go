...
func Generate(infrastructure []byte, rawData map[string][]byte, envNum string, topicIpRange, topicPortRange, tcpPortRange []string) (map[string][]byte, error) {
	...
	for k, _ := range rawData {
		if len(rawData[k]) == 0 && !strings.Contains(k, define.Template) {
			delete(rawData, k)
		}
	}
	...
	//生成topicInfo总表
	topicInfoList, hostTcpPortMap, tcpRangeSli, err := GenerateTopicInfo(dplyStructList, rawSlice, envNum, topicIpRange, topicPortRange, tcpPortRange)
	if err != nil {
		return nil, err
	}
	...
	for _, platformIns := range dplyStructList {
		for _, nodeTypeIns := range platformIns.NodeTypeList {
			...
			for _, setIns := range nodeTypeIns.SetList {
				...
				pathDp := commonPath + "/" + define.Deployment
				...
				if isTpcExst {
					pathTpc := commonPath + "/" + define.TopicInfo
