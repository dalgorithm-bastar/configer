...
func GenerateTopicInfo(dplyStructList []ChartDeployMain, rawSlice []RawFile, ipRange []string, portRange []string, envNum string) (map[string]map[string]map[string]ExpTpcMain, error) {
	...
	for _, fileInfo := range rawSlice {
		...
		if len(pathSlice) < 6 {
			return nil, errors.New(fmt.Sprintf("service.yaml in wrong place ,with path %s", path))
		}
		//读取并归档当前文件的服务声明
		var srvStruct SrvMain
		err := yaml.Unmarshal(data, &srvStruct)
		if err != nil {
			return nil, fmt.Errorf("load service.yaml err, file path:%s, err:%s", path, err.Error())
		}
		//检测当前平台或节点类型是否收录过
		...
		for _, nodeTypeIns := range platInfo.NodeTypes {
			...
			for _, netUnit := range SrvIns.PubTopic {
				...
				for _, setUnit := range netUnit.SetTopic {
					...
					if len(sli) != 2 {
						return nil, errors.New(fmt.Sprintf("setTopicName err:\"%s\" in nodetype %s", setTpcName, nodeTypeName))
					}
