...
//用于主机tcp端口池，因为所有主机共用一个tcp端口段，所以不在每个结构体中记录端口段
...
func GenerateTopicInfo(dplyStructList []ChartDeployMain, rawSlice []RawFile, envNum string, topicIpRange, topicPortRange, tcpPortRange []string) (map[string]map[string]map[string]ExpTpcMain, map[string]hostTcpUnit, []int, error) {
	...
	for _, fileInfo := range rawSlice {
		...
		if len(pathSlice) < 6 {
			return nil, nil, nil, errors.New(fmt.Sprintf("service.yaml in wrong place ,with path %s", path))
		}
		//读取并归档当前文件的服务声明
		...
		if isPlatExist && isNodeTypeExist {
			...
			if mainSlice[platIdx].NodeTypes[nodeTypeIdx].SrvFile.InnerTopicNet != "" {
				if srvStruct.InnerTopicNet != mainSlice[platIdx].NodeTypes[nodeTypeIdx].SrvFile.InnerTopicNet {
					return nil, nil, nil, errors.New(fmt.Sprintf("Multi inner topic net assigned in nodeType: %s", path))
				}
			...
			if err != nil {
				return nil, nil, nil, errors.New(fmt.Sprintf("merge srv err in path: %s;", path) + err.Error())
			}
	...
	//初始化ip、port生成
	_, seedRanges, err := FindIpv4Seeds(topicIpRange)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("topicIpRange err: %s", err.Error())
	}
	topicPortRanges, err := sortPorts(topicPortRange)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("topicPortRange err: %s", err.Error())
	}
	tcpPortRanges, err := sortPorts(tcpPortRange)
	if err != nil {
		return nil, nil, nil, fmt.Errorf("tcpPortRange err: %s", err.Error())
	}
	seed, actPort, coverPort := seedRanges[0], topicPortRanges[0], strconv.Itoa(topicPortRanges[0])
	seedIdx, portIdx := 0, 0
		...
		for _, nodeTypeIns := range platInfo.NodeTypes {
			...
			for _, netUnit := range SrvIns.PubTopic {
				...
				for _, bizUnit := range netUnit.BizTopic {
					if bizUnit.TpcName == "" {
						return nil, nil, nil, fmt.Errorf("nil bizTopic got under service of: %s.%s", platName, nodeTypeName)
					}
						...
						if netMap[netName][i].Name == bizTpcName {
							...
							for _, pubSetIns := range dplyStructList[pubPi].NodeTypeList[pubNi].SetList {
								...
								for _, pubNodeIns := range pubSetIns.Deployment.Node {
									//检测节点上是否具有该网络
									isNetExist, netIdx := checkNetOnNode(netName, pubNodeIns.Network)
									if !isNetExist {
										return nil, nil, nil, errors.New(fmt.Sprintf("node in set: %s have no adapter for net: %s for topic: %s", util.Join("/", platName, nodeTypeName, pubSetIns.SetName), netName, bizTpcName))
									}
								...
								for _, subUnit := range netMap[netName][i].Sub {
									...
									for _, subSetIns := range dplyStructList[subPi].NodeTypeList[subNi].SetList {
										...
										for _, subNodeIns := range subSetIns.Deployment.Node {
											...
											if !isNetExst {
												return nil, nil, nil, errors.New(fmt.Sprintf("node in set: %s have no adapter for net: %s for topic: %s", util.Join("/", subUnit.Plat, subUnit.NodeType, subSetIns.SetName), netName, bizTpcName))
											}
				...
				for _, setUnit := range netUnit.SetTopic {
					...
					if len(sli) != 2 {
						return nil, nil, nil, errors.New(fmt.Sprintf("setTopicName err:\"%s\" in nodetype %s", setTpcName, nodeTypeName))
					}
					isSubDploy, subpi, subni := checkDeployment(dplyStructList, sli[0], sli[1])
					if !isSubDploy {
						return nil, nil, nil, errors.New(fmt.Sprintf("sub undeployed:%s", setTpcName))
					}
					...
					for _, pubSetIns := range dplyStructList[pubPi].NodeTypeList[pubNi].SetList {
						for _, subSetIns := range dplyStructList[subpi].NodeTypeList[subni].SetList {
							...
							for _, pubNode := range pubSetIns.Deployment.Node {
								...
								if !isNetExst {
									return nil, nil, nil, errors.New(fmt.Sprintf("node in set: %s have no adapter for net: %s for topic: %s", util.Join("/", platName, nodeTypeName, pubSetIns.SetName), netName, setTpcName))
								}
							...
							insertTpcInfo(topicInfoMap, pubTpcInfo, platName, nodeTypeName, pubSetIns.SetName)
							//在发送方插入完毕后取接收方信息，保证发给自身的情况下的正确性
					...
					if i < len(setIns.Deployment.Node) {
						...
						if _, ok := hostTcpPoolMap[hostName]; !ok {
							hostTcpPoolMap[hostName] = hostTcpUnit{
								actualTcpPort: uint16(tcpPortRanges[0]),
								PortIdx:       0,
								coverTcpMap:   make(map[int]int),
							}
						}
						actualListenPort := int(hostTcpPoolMap[hostName].actualTcpPort)
						actualListenPort, coverListenPort, currentIdx, listenPortOverFlow := getNextListenPort(tcpPortRanges, int(hostTcpPoolMap[hostName].PortIdx), actualListenPort, envNum, hostTcpPoolMap[hostName].coverTcpMap)
						if listenPortOverFlow {
							return nil, nil, nil, fmt.Errorf("listenPort used up on host: %s", hostName)
						}
						//更新端口池
						tmp := hostTcpUnit{
	...
	for _, bizVals := range netMap {
		for _, bizVal := range bizVals {
			if !bizVal.IsProc {
				return nil, nil, nil, errors.New(fmt.Sprintf("bizTopic: %s subed but not pub", bizVal.Name))
			}
		}
	}
	return topicInfoMap, hostTcpPoolMap, tcpPortRanges, nil
}
...
func getNextListenPort(ports []int, idxin int, inputPort int, envNum string, listenPortMap map[int]int) (actualListenPort int, coverPort string, idx int, overFlow bool) {
	...
	for !isCoverPortInRange {
		...
		for i := 0; i < len(ports)/2; i++ {
			//满足要求
			if coverPortInt >= ports[2*i] && coverPortInt <= ports[2*i+1] {
				isCoverPortInRange = true
...
func getNextEndPoint(idxI int, seedRanges []int32, oldSeed int32, ports []int, idxP int, envNum string, actualPort int, coverPortIn string, ipMap map[string]int, portMap map[int]int) (int, int32, int, int, string, string, bool) {
	isIpRpt, isIpOverFlow, isPortOverflow, idxPNew := true, false, false, 0
	ipv4, res := "", ""
	for isIpRpt {
	...
	if !isIpOverFlow {
		actualPort, coverPortIn, idxPNew, isPortOverflow = getNextListenPort(ports, idxP, actualPort, envNum, portMap)
		if isPortOverflow {
			//溢出
			return 0, 0, 0, 0, "", "", true
		}
		//在已排除溢出的情况下，擦除portMap中相应项，再回退相关值
		coverPort, _ := strconv.Atoi(coverPortIn)
		delete(portMap, coverPort)
		if idxPNew != idxP {
			actualPort = ports[2*idxP+1]
			idxPNew--
		} else {
			actualPort--
		}
	...
	return idxI, oldSeed, idxPNew, actualPort, coverPortIn, res, false
}
