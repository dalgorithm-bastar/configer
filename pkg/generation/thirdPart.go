...
func huaRuiMain(infrastructure []byte, deployment []ChartDeployMain, topicList []byte, envNum string) (map[string][]byte, error) {
    ...
    routeCfgData := huaRuiGetRouteCfg(topicList, envNum, newLine)
...
func huaRuiGetHostCfg(infrastructure []byte, newLine string) []byte {
    ...
    _ = json.Unmarshal(infrastructure, &infraStruct)
...
func huaRuiGetRouteCfg(topicList []byte, envNum, newLine string) []byte {
            ...
            for _, setIns := range nodeTypeIns.SetList {
                ...
                for _, bizTopicIns := range setIns.BroadInfo.SubExtern.BizTopic {
                    ...
                    msg := util.Join("|", "1", pubSetSlice[1], strconv.Itoa(int(setIdPub)), nodeTypeSub,
                        strconv.Itoa(int(setIdSub)), bizTopicIns.EndPoint+"@@"+envNum, strconv.Itoa(int(bizTopicIns.TopicId)), isRmb)
                ...
                for _, setTopicIns := range setIns.BroadInfo.SubExtern.SetTopic {
                    ...
                    msg := util.Join("|", "1", pubSetSlice[1], strconv.Itoa(int(setIdPub)), nodeTypeSub,
                        strconv.Itoa(int(setIdSub)), setTopicIns.EndPoint+"@@"+envNum, strconv.Itoa(int(setTopicIns.TopicId)), isRmb)
                    if _, ok := msgMap[msg]; !ok {
                        msgMap[msg] = 0
                    }
                }
