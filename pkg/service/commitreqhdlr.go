...
func commit(ctxRoot context.Context, req *pb.CfgReq) (error, []*pb.VersionInfo, *pb.AnyFile) {
    ...
    err = repository.Src.AcidCommit(fileMap, deleteKeySlice)
    if err != nil {
        log.Sugar().Errorf("AcidCommit err of %v when posting, putmap:%+v, deleteSlice:%+v", err, fileMap, deleteKeySlice)
        return err, nil, nil
    }
    defer func() {
        if mutex != nil {
