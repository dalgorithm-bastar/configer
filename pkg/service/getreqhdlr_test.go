...
func Test_getCache(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want:  errors.New("get permfile err:get perm err, filepath:qrchen/perm.yaml"),
			want2: nil,
			want1: nil,
		},
		{
			...
			want:  errors.New("get permfile err:<nil>, filepath:qrchen/perm.yaml"),
			want2: nil,
			want1: nil,
		},
		{
	...
	gomock.InOrder(
		...
		mockSrc.EXPECT().Get(gomock.Any()).Return(nil, errors.New("get perm err")),
		mockSrc.EXPECT().GetbyPrefix(gomock.Any()).Return(map[string][]byte{"test": []byte("test")}, nil),
		mockSrc.EXPECT().Get(gomock.Any()).Return(nil, nil),
		mockSrc.EXPECT().GetbyPrefix(gomock.Any()).Return(map[string][]byte{"test": []byte("test")}, nil),
		mockSrc.EXPECT().Get(gomock.Any()).Return([]byte("test perm"), nil),
		mockSrc.EXPECT().GetbyPrefix(gomock.Any()).Return(map[string][]byte{"test": []byte("test")}, nil),
		mockSrc.EXPECT().Get(gomock.Any()).Return([]byte("test perm"), nil),
	)
...
func Test_getConfig(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want:  errors.New("get permfile err:get perm err, filepath:1.0.0/perm.yaml"),
			want2: nil,
			want1: nil,
		},
		{
			...
			want:  errors.New("get permfile err:<nil>, filepath:1.0.0/perm.yaml"),
			want2: nil,
			want1: nil,
		},
		{
		...
		},
		{
			...
			want:  errors.New("generate err:generate err, or get nil generation files"),
			want2: nil,
			want1: nil,
		},
		{
	...
	outputsInfra := []gomonkey.OutputCell{
		...
		{Values: gomonkey.Params{nil, nil, &pb.AnyFile{
			FileName: define.Infrastructure,
			FileData: []byte("test"),
		}}},
		{Values: gomonkey.Params{nil, nil, &pb.AnyFile{
			FileName: define.Infrastructure,
	...
	outputsGen := []gomonkey.OutputCell{
		...
		{Values: gomonkey.Params{make(map[string][]byte), nil}},
		{Values: gomonkey.Params{make(map[string][]byte), nil}},
	}
	...
	outputsEnvNum := []gomonkey.OutputCell{
		//{Values: gomonkey.Params{errors.New("update envNum err")}},
		{Values: gomonkey.Params{nil}},
	}
	patchesEnvNum := gomonkey.ApplyFuncSeq(updateEnvNumFile, outputsEnvNum)
	defer patchesEnvNum.Reset()

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, got1, got2 := getConfig(tt.args.ctx, tt.args.req, tt.args.infra)
			if !reflect.DeepEqual(got, tt.want) {
...
func Test_getRaw(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want:  errors.New("get permfile err:get perm err, filepath:/perm.yaml"),
			want2: nil,
			want1: nil,
		},
		{
			...
			want:  errors.New("get permfile err:<nil>, filepath:/perm.yaml"),
			want2: nil,
			want1: nil,
		},
		{
...
func Test_getVersion(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			want:  errors.New("get unexpected version data from database,data:test, please checkout"),
			want2: nil,
			want1: nil,
		},
		{
	...
	gomock.InOrder(
		...
		mockSrc.EXPECT().Get(gomock.Any()).Return(nil, nil),
		mockSrc.EXPECT().Get(gomock.Any()).Return([]byte("test"), nil),
		mockSrc.EXPECT().Get(gomock.Any()).Return([]byte("1.0.0,qrchen,time"), nil),
	)
...
func Test_updateEnvNumFile(t *testing.T) {
	genSrc := GenSrc{
		UserName:       "qrchen",
		Version:        "3.1.0",
		Scheme:         "scheme1",
	...
	}{
		{
			...
			args: args{
				req: &pb.CfgReq{
					...
					ArgRange: &pb.ArgRange{
						TopicIp:   []string{"1.1.1.1", "2.2.2.2"},
						TopicPort: []string{"1024", "65535"},
						TcpPort:   []string{"1024", "65535"},
					},
				},
				infra: nil,
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := updateEnvNumFile(tt.args.req, tt.args.infra); (err != nil) != tt.wantErr {
				t.Errorf("updateEnvNumFile() error = %v, wantErr %v", err, tt.wantErr)
			}
		})
	}
}
