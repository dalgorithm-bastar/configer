...
配置中心实现对配置文件的托管、版本化管理和动态生成。在接到生成请求时，配置中心按照本地输入或服务端存储的基础设施信息文件、部署信息文件、服务信息文件的要求，分配各个集群的集群ID、节点ID等参数，在确定部署参数后为每个集群分配组播信息，然后使用这些信息中的一部分，填充各个节点类型的配置模板，生成对应于每个节点的配置文件，最终将生成的配置文件打包返回给用户。
