...
| 序号 | 修订日期   | 修订人 | 审核人 | 修订来源                 | 修订内容概述       |
| ---- | ---------- | ------ | ------ | ------------------------ | ------------------ |
| 1    | 2022/08/30 | 陈清睿 | 韩大伟 | WBS-1159配置中心开发需求 | 修改服务信息字段表 |
...
|   isRMB   | *uint16 |             判断该组播是否可靠的标志位，uint16值为1时表示可靠，其余值由用户自定义             |
| unaidedIO |  *bool  |                         判断该组播是否是非阻塞IO，bool值由用户自定义                          |
| priority  | *uint16 |           判断该组播优先级的标志位，uint16值为1时表示可靠，字段uint16值由用户自定义           |
...
| userName |    string    | 指定部署该集群时免密连接使用的用户名，可用@@作为通配符，生成配置时将使用后台环境号将其替换 |
|  setID   |    uint16    |                              指定集群的分区号，人工保证不重复                              |
...
文件中包含的所有id和index都从1开始自增，仅在集群内通信的"main"组播通道信息内会出现0值。
