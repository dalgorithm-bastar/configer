...
```go
//函数原型
func(mode, clusterObject, service string) (string, error){}

//mode, 可选填“slice”或者“normal”
//填“slice”时，表示使用localId作为索引，并在部署信息中查找
//其他场景下应填“normal”
...
[rmb_publisher]
TOPIC_TYPE = "1"
HOST = {{GetInfo "slice" "DTP.MC.set0" "deployment_info.host"}} #要替换的部分
```
...
```go
//函数原型
func(mode, anyVersion, anyEnv, clusterObject, service string) (string, error) {}
...
[rmb_publisher]
TOPIC_TYPE = "1"
HOST = {{UnsafeGetInfo "slice" "0.0.1" "00" "DTP.MC.set0" "deployment_info/host"}} #要替换的部分
```
...
//NodeId
//目标节点号，取决于该集群部署信息中数组索引(注意，是数组索引，不是自定义id)
